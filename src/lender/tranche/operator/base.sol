...
contract BaseOperator is Math, DSNote, Auth {
    ...
    function _supply(uint currencyAmount) internal {
        ...
        tranche.supply(msg.sender, currencyAmount, rdiv(currencyAmount, assessor.calcUpdatedTokenPrice(address(tranche))));
        distributor.balance();
    }

    function _redeem(uint tokenAmount) internal {
        distributor.balance();
        uint currencyAmount = rmul(tokenAmount, assessor.calcUpdatedTokenPrice(address(tranche)));
